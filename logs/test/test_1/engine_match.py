#!/usr/bin/env python3
"""
Engine vs Engine - Pure UCI + Draw Detection + Batch Mode
Configurable: seconds per move, games per matchup, max moves.

Usage:
    python engine_match.py              # default: 1s/move, 1 game each
    python engine_match.py 5 10         # 5s/move, 10 games each matchup
    python engine_match.py 10 5 300     # 10s/move, 5 games, 300 max moves
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# CONFIG (overridden by command line args)
# ═══════════════════════════════════════════════════════════════════════════════
...
MOVE_TIME_SEC = 1.0       # seconds per move
GAMES_PER_MATCHUP = 1     # games per color/variant combination
MAX_MOVES = 200           # max half-moves per game

REPETITION_THRESHOLD = 3
MAX_NO_CAPTURE_PLY = 100  # 50-move rule = 100 half-moves
...
def run_matchup(white_name, white_opts, black_name, black_opts,
    ...
    """Run N games for a single matchup, return stats."""
...
def main():
    ...
    # Parse command line: [seconds_per_move] [games_per_matchup] [max_moves]
    if len(sys.argv) > 1:
    ...
    # Matchup 1: Standard - Original vs Swapped
    stats = run_matchup("Original", ORIGINAL_OPTIONS, "Swapped", SWAPPED_OPTIONS,
                        "chess", "standard", MAX_MOVES, movetime_ms)
    all_stats["Standard: O vs S"] = stats

    # Matchup 2: Atomic - Original vs Swapped
    stats = run_matchup("Original", ORIGINAL_OPTIONS, "Swapped", SWAPPED_OPTIONS,
                        "atomic", "atomic", MAX_MOVES, movetime_ms)
    all_stats["Atomic: O vs S"] = stats

    # Matchup 3: Standard Reverse
    stats = run_matchup("Swapped", SWAPPED_OPTIONS, "Original", ORIGINAL_OPTIONS,
                        "chess", "standard_rev", MAX_MOVES, movetime_ms)
    all_stats["Standard: S vs O"] = stats

    # Matchup 4: Atomic Reverse
    stats = run_matchup("Swapped", SWAPPED_OPTIONS, "Original", ORIGINAL_OPTIONS,
                        "atomic", "atomic_rev", MAX_MOVES, movetime_ms)
    all_stats["Atomic: S vs O"] = stats
    ...
    for matchup, stats in all_stats.items():
        ...
        u = stats.get("*", 0)
        print(f"\n  {matchup}")
        print(f"    White wins: {w}/{total}  |  Black wins: {b}/{total}  |  Draws: {d}/{total}  |  Unknown: {u}/{total}")
        if total > 0:
