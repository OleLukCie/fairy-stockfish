#!/usr/bin/env python3
r"""
E2 Match: Scale Sweep - Test all 3 scaled models vs Original + Swapped
Usage:
    python e2_match.py [seconds_per_move] [games_per_matchup] [max_moves]
"""
...
OUTPUT_DIR = BASE_DIR / "data" / "E2"

E2_DIR = BASE_DIR / "NNUE" / "E2"
ORIGINAL_DIR = BASE_DIR / "NNUE" / "Original"
SWAPPED_DIR = BASE_DIR / "NNUE" / "Swapped"

# Three scaled variants
SCALE_VARIANTS = {
    "Scale0.5": str(E2_DIR / "atomic_scale0.5.nnue"),
    "Scale1.5": str(E2_DIR / "atomic_scale1.5.nnue"),
    "Scale2.0": str(E2_DIR / "atomic_scale2.0.nnue"),
}
...
def main():
    ...
    print("=" * 70)
    print("  E2: Scale Sweep Experiment - Atomic Matches")
    print(f"  Time: {MOVE_TIME_SEC}s/move | Games: {GAMES_PER_MATCHUP} | Max moves: {MAX_MOVES}")
    print("=" * 70)
    print("\n  Hypothesis: If scale 2.0 performs best,")
    ...
    for scale_name, model_path in SCALE_VARIANTS.items():
        print(f"\n{'='*70}")
        ...
        # vs Original
        stats = run_matchup(scale_name, scale_options, "Original", ORIGINAL_OPTIONS,
                            "atomic", f"{scale_name.lower()}_vs_original", MAX_MOVES, movetime_ms)
        all_stats[f"{scale_name} vs Original"] = stats

        # vs Swapped
        stats = run_matchup(scale_name, scale_options, "Swapped", SWAPPED_OPTIONS,
                            "atomic", f"{scale_name.lower()}_vs_swapped", MAX_MOVES, movetime_ms)
        all_stats[f"{scale_name} vs Swapped"] = stats

        # Reverse: Original vs Scale
        stats = run_matchup("Original", ORIGINAL_OPTIONS, scale_name, scale_options,
                            "atomic", f"original_vs_{scale_name.lower()}", MAX_MOVES, movetime_ms)
        all_stats[f"Original vs {scale_name}"] = stats

        # Reverse: Swapped vs Scale
        stats = run_matchup("Swapped", SWAPPED_OPTIONS, scale_name, scale_options,
                            "atomic", f"swapped_vs_{scale_name.lower()}", MAX_MOVES, movetime_ms)
        all_stats[f"Swapped vs {scale_name}"] = stats
    ...
    for scale_name in SCALE_VARIANTS.keys():
        print(f"\n  --- {scale_name} ---")

        # vs Original
        fwd = all_stats.get(f"{scale_name} vs Original", {})
        rev = all_stats.get(f"Original vs {scale_name}", {})
        total_games = sum(fwd.values()) + sum(rev.values())
        wins = fwd.get("1-0", 0) + rev.get("0-1", 0)
        print(f"    vs Original: {wins}/{total_games} wins ({wins/total_games*100:.1f}%)")

        # vs Swapped
        fwd = all_stats.get(f"{scale_name} vs Swapped", {})
        rev = all_stats.get(f"Swapped vs {scale_name}", {})
        total_games = sum(fwd.values()) + sum(rev.values())
        wins = fwd.get("1-0", 0) + rev.get("0-1", 0)
        print(f"    vs Swapped:  {wins}/{total_games} wins ({wins/total_games*100:.1f}%)")
