#!/usr/bin/env python3
r"""
E1 Match: Bias Recentered Atomic vs Original Atomic + Swapped
Usage:
    python e1_match.py [seconds_per_move] [games_per_matchup] [max_moves]
    python e1_match.py 5 10        # 5s/move, 10 games each
"""
...
OUTPUT_DIR = BASE_DIR / "data" / "E1"

# E1: Bias recentered atomic model
E1_DIR = BASE_DIR / "NNUE" / "E1"
ORIGINAL_DIR = BASE_DIR / "NNUE" / "Original"
SWAPPED_DIR = BASE_DIR / "NNUE" / "Swapped"

E1_OPTIONS = {
    "EvalFile": str(E1_DIR / "atomic-2cf13ff256cc.nnue"),
...
def main():
    ...
    print("=" * 70)
    print("  E1: Bias Recentering Experiment - Atomic Matches")
    print(f"  Time: {MOVE_TIME_SEC}s/move | Games: {GAMES_PER_MATCHUP} | Max moves: {MAX_MOVES}")
    print("=" * 70)
    print("\n  Hypothesis: If bias recentering improves performance,")
    ...
    # Matchup 1: E1 (W) vs Original Atomic (B)
    stats = run_matchup("E1_Bias0", E1_OPTIONS, "Original", ORIGINAL_OPTIONS,
                        "atomic", "e1_vs_original", MAX_MOVES, movetime_ms)
    all_stats["E1 vs Original"] = stats

    # Matchup 2: Original Atomic (W) vs E1 (B)
    stats = run_matchup("Original", ORIGINAL_OPTIONS, "E1_Bias0", E1_OPTIONS,
                        "atomic", "original_vs_e1", MAX_MOVES, movetime_ms)
    all_stats["Original vs E1"] = stats

    # Matchup 3: E1 (W) vs Swapped (B) - key test!
    stats = run_matchup("E1_Bias0", E1_OPTIONS, "Swapped", SWAPPED_OPTIONS,
                        "atomic", "e1_vs_swapped", MAX_MOVES, movetime_ms)
    all_stats["E1 vs Swapped"] = stats

    # Matchup 4: Swapped (W) vs E1 (B) - key test!
    stats = run_matchup("Swapped", SWAPPED_OPTIONS, "E1_Bias0", E1_OPTIONS,
                        "atomic", "swapped_vs_e1", MAX_MOVES, movetime_ms)
    all_stats["Swapped vs E1"] = stats
    ...
    # Key comparison
    print("\n" + "=" * 70)
    print("  KEY COMPARISON: E1 vs Swapped")
    print("=" * 70)
    e1_total_wins = all_stats["E1 vs Swapped"].get("1-0", 0) + all_stats["Swapped vs E1"].get("0-1", 0)
    e1_total_games = sum(all_stats["E1 vs Swapped"].values()) + sum(all_stats["Swapped vs E1"].values())
    print(f"  E1 wins vs Swapped: {e1_total_wins}/{e1_total_games} ({e1_total_wins/e1_total_games*100:.1f}%)")

    orig_total_wins = all_stats.get("Atomic: O vs S", {}).get("1-0", 0) + all_stats.get("Atomic: S vs O", {}).get("0-1", 0)
    # We didn't run Original vs Swapped in this script, but user has data from test_1
